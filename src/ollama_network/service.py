...
from .constants import APP_VERSION, DEFAULT_PUBLIC_SERVER_URL
...
class NetworkService:
    ...
    def download_worker_launcher(self, actor_user_id: str) -> tuple[str, bytes]:
        normalized_user_id = str(actor_user_id or "").strip()
        if not normalized_user_id:
            raise ValueError("actor_user_id is required.")
        filename = f"start_llm_network_worker_v{APP_VERSION.replace('.', '_')}.bat"
        content = "\r\n".join(
            [
                "@echo off",
                "setlocal",
                ...
                ")",
                "",
                f"echo LLM Network worker launcher v{APP_VERSION}",
                "echo.",
